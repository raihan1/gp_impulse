    ...
    <div class="page-content">
                ...
                <div class="form-body">
                    <div class="form-group">
                        <div class="col-md-5 col-sm-5 col-xs-12">
                            ...
                            <label class="col-md-4 col-sm-4 col-xs-12 control-label">Project</label>
                            <div class="col-md-8 col-sm-8 col-xs-12 form-group" id="projectContainer">
                                <?php
                                echo $this->Form->input( 'project', array(
                                    ...
                                    'class'   => 'form-control',
                                    'id'      => 'project',
                                    'value'   => !empty( $data['Ticket']['project'] ) ? $data['Ticket']['project'] : '',
                                ) );
                                ?>
                            </div>
                        ...
                        <div class="col-md-7 col-sm-7 col-xs-12">
                            ...
                            <div class="col-md-8 col-sm-8 col-xs-12 form-group">
                                <div style="width: 100% !important;" class="input-group input-medium" data-date-format="dd-mm-yyyy" data-date-start-date="+0d">
                                    <?php
        ...
        <div class="row">
            <div class="col-md-12 text-center margin-bottom-20">
                ...
                <?php echo $this->Html->link( '<i class="fa fa-arrow-left"></i> Cancel', array( 'plugin' => 'tr_validation', 'controller' => 'tickets', 'action' => 'index' ), array( 'escape' => FALSE, 'class' => 'btn red' ) ); ?>
            </div>
        </div>
...
<script type="text/javascript">
    ...
    $( document ).ready( function() {
        ...
        gp_warranty.select_options( 'project' );
        ...
        $( '#site' ).on( 'change', function() {
            ...
            $.ajax( {
                ...
                success    : function( data ) {
                    ...
                    var project_options = '<select name="data[Ticket][project]" class="form-control" id="project">'
                    if( site_id == 0 || site_id == '' || typeof site_id == 'undefined' ) {
                        project_options += '<option value="">Select a project</option>';
                    }
                    if( data[ 'Project' ].length > 0 ) {
                        for( var i = 0; i < data[ 'Project' ].length; i++ ) {
                            project_options += '<option value="' + data[ 'Project' ][ i ][ 'Project' ][ 'project_name' ] + '" data-id="' + data[ 'Project' ][ i ][ 'Project' ][ 'id' ] + '"' + ( project == data[ 'Project' ][ i ][ 'Project' ][ 'project_name' ] ? ' selected' : '' ) + '>' + data[ 'Project' ][ i ][ 'Project' ][ 'project_name' ] + '</option>';
                        }
                    }
                    project_options += '</select>';
                    $( '#projectContainer' ).html( project_options );
                    gp_warranty.select_options( 'project' );
...
//                    gp_warranty.select_options( 'asset_number' );
        ...
        /*New Asset Number filter*/
                $( document ).on( 'change', '#asset_number', function() {
                    $( '.fancybox-loading' ).show();
                    $( '.mask' ).show();

                    var asset_number_id = $( this ).find( 'option:selected' ).attr( 'data-id' );
                    var site_id = $( '#site' ).find( 'option:selected' ).attr( 'data-id' );

                    $.ajax( {
                        dataType   : 'json',
                        type       : 'POST',
                        evalScripts: true,
                        url        : '<?php echo Router::url( array( 'controller' => 'tickets', 'action' => 'asset_number_selected' ) ); ?>',
                        data       : {asset_number_id :asset_number_id,site_id : site_id},
                        success    : function( data ) {

                            $( '#assetGroupContainer' ).html( '' );

                            var asset_group_options = '<select name="data[Ticket][asset_group]" class="form-control" id="asset_group">'
                            if( site_id == 0 || site_id == '' || typeof site_id == 'undefined' ) {
