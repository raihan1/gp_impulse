...
foreach( $data as $d ) {
    $result['data'][] = array(
        ...
        $this->Html->link( '<i class="fa fa-eye"></i> View', array( 'plugin' => 'supplier', 'controller' => 'tickets', 'action' => 'view', $d['Ticket']['id'] ), array( 'escape' => FALSE, 'class' => 'btn btn-xs green', 'title' => 'View Detail' ) ),
    );
}
