 ...
 * Tickets Controller
 */
class TicketsController extends TrValidationAppController {
    
    public $uses = array( 'User', 'Site', 'Ticket', 'Project', 'AssetGroup', 'AssetNumber', 'TrClass', 'Supplier', 'SupplierCategory', 'Service', 'TrService', 'SubCenter', 'SubCenterBudget' );
    ...
    public function index() {
        ...
        $this->set( array(
            'total'            => number_format( $data[0]['total_with_vat'], 4 ),
    ...
    public function data( $type ) {
        ...
        //<editor-fold desc="Group actions (sub-total)" defaultstate="collapsed">
            ...
            if( $this->request->data['customActionName'] == 'is_subtotal' ) {
                $data = $this->Ticket->find( 'first', array(
                    'conditions' => array(
                        ...
                        'Ticket.is_invoiceable'  => NO,
                        'Ticket.id'              => $this->request->data['id'],
                    ),
                    'contain'    => FALSE,
                    'fields'     => array( 'SUM(Ticket.total_with_vat) AS total' ),
                ) );
                $this->set( 'subTotal', $data[0]['total'] );
            }
        }
        //</editor-fold>
        ...
        foreach( $columns as $col ) {
            if( isset( $this->request->data[ $col['searchName'] ] ) && $this->request->data[ $col['searchName'] ] != '' && !is_array( $this->request->data[ $col['searchName'] ] ) ) {
...
//                    die(pr($this->request->data['Ticket']['batch']));
                ...
                if( empty( $errors ) ) {
                    $this->redirect( array( 'plugin' => 'tr_validation', 'controller' => 'tickets', 'action' => 'index', '#' => 'assigned' ) );
                }
                else {
...
//        }
    ...
    /**
     * View a ticket details (if pending, on submit reject)
     *
     * @param integer $trId
     *
    ...
    public function view( $trId = NULL ) {
        $data = $this->Ticket->find( 'first', array(
            ...
            'contain'    => array(
                'TrService' => array(
                    ...
                    'LastService' => array( 'fields' => array( 'delivery_date', 'quantity' ) ),
                ),
            ),
        ) );
        if( empty( $data ) ) {
        ...
        /* Reject ticket. Approve is handled by approve() via ajax. */
        if( $this->request->is( array( 'post', 'put' ) ) ) {
            try {
                //<editor-fold desc="Save services" defaultstate="collapsed">
                foreach( $this->request->data['TrService'] as $trs ) {
                    $trServiceData = $this->TrService->find( 'first', array( 'conditions' => array( 'TrService.id' => $trs['id'] ), 'contain' => FALSE ) );
                    
                    $latestComment = empty( $trs['comments'] ) ? '' : ( '<b>' . $this->loginUser['User']['name'] . "</b>: {$trs['comments']} (" . date( 'Y-m-d H:i:s' ) . ')' );
                    $comments = $trServiceData['TrService']['comments'] . ( !empty( $trServiceData['TrService']['comments'] ) && !empty( $latestComment ) ? '<br />' : '' ) . $latestComment;
                ...
                $this->Ticket->create();
                if( !$this->Ticket->save( $saveData, array( 'fieldList' => array( 'approval_status', 'validation_date', 'tr_validation_by' ) ) ) ) {
                ...
                if( $this->request->is( 'ajax' ) ) {
                    ...
                    $this->redirect( array( 'plugin' => 'tr_validation', 'controller' => 'tickets', 'action' => 'index', '#' => 'pending' ) );
                }
            }
            catch( Exception $e ) {
                if( $this->request->is( 'ajax' ) ) {
    ...
    public function calculate_complete_date() {
        ...
        $h = !empty( $data['TrClass']['no_of_days'] ) ? ( 24 * $data['TrClass']['no_of_days'] ) : 0;
        $completeDate = date( 'Y-m-d H:i:s', strtotime( '+ ' . round( $h, 0 ) . ' hours', strtotime( $this->request->data['received_at_supplier'] ) ) );
        
        die( json_encode( $completeDate ) );
    }
    
    /**
     * Approve ticket via ajax
     *
     * @param null|integer $trId
     */
    public function approve( $trId = NULL ) {
        try {
            $this->autoRender = FALSE;
            
            $ticket = $this->Ticket->find( 'first', array( 'conditions' => array( 'Ticket.id' => $trId ), 'contain' => FALSE ) );
            if( empty( $ticket ) ) {
                throw new Exception( 'Invalid ticket ID.' );
            }
            ...
            $saveData = array( 'Ticket' => array(
                ...
                'validation_date'  => date( 'Y-m-d H:i:s' ),
                'tr_validation_by' => $this->loginUser['User']['id'],
                'stage'            => INVOICE_CREATION_STAGE,
            ) );
            $this->Ticket->create();
            if( !$this->Ticket->save( $saveData, array( 'fieldList' => array( 'approval_status', 'validation_date', 'tr_validation_by', 'stage' ) ) ) ) {
                throw new Exception( 'Failed to save ticket data.' );
            }
            $this->Session->setFlash( __( 'Ticket updated successfully.' ), 'messages/success' );
            //</editor-fold>
            ...
            $mainTypes = Configure::read( 'mainTypes' );
            $mainType = $this->WarrantyLookup->getMainType( $ticket['Ticket']['tr_class'] );
            ...
            if( in_array( $mainType, $mainTypes ) ) {
                ...
                $this->SubCenterBudget->updateAll(
                    array( $column => $column . ' + ' . $ticket['Ticket']['total_with_vat'] ),
                    array(
                        'sub_center_id' => $this->loginUser['User']['sub_center_id'],
                        'month'         => date( 'm' ),
                        'year'          => date( 'Y' ),
