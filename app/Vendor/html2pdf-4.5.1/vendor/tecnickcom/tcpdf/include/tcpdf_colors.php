	...
	public static function convertHTMLColorToDec($hcolor, &$spotc, $defcol=array('R'=>128,'G'=>128,'B'=>128)) {
		...
		if ($color[0] != '#') {
			...
			if (isset(self::$webcolor[$color])) {
				// web color
				$color_name = self::$webcolor[$color];
			} else {
				// spot color
		...
		switch (strlen($color_name)) {
			case 3: {
				// 3-digit RGB hexadecimal representation
				$r = substr($color_name, 0, 1);
				$g = substr($color_name, 1, 1);
				$b = substr($color_name, 2, 1);
				$returncolor = array();
			...
			case 6: {
				...
				$returncolor = array();
				$returncolor['R'] = max(0, min(255, hexdec(substr($color_name, 0, 2))));
				$returncolor['G'] = max(0, min(255, hexdec(substr($color_name, 2, 2))));
				$returncolor['B'] = max(0, min(255, hexdec(substr($color_name, 4, 2))));
				break;
			}
			case 8: {
				// 8-digit CMYK hexadecimal representation
				$returncolor = array();
				$returncolor['C'] = max(0, min(100, round(hexdec(substr($color_name, 0, 2)) / 2.55)));
				$returncolor['M'] = max(0, min(100, round(hexdec(substr($color_name, 2, 2)) / 2.55)));
				$returncolor['Y'] = max(0, min(100, round(hexdec(substr($color_name, 4, 2)) / 2.55)));
				$returncolor['K'] = max(0, min(100, round(hexdec(substr($color_name, 6, 2)) / 2.55)));
				break;
			}
			default: {
				$returncolor = $defcol;
