...
/**
 * CakePHP (version 2) component to upload, resize, crop and add watermark to images.
...
class QimageComponent extends Component {
    ...
    public function initialize(Controller $controller) {
        $this->watermarkImage = 'img' . DS . 'watermark.png';
        $this->jpgQuality = 100;
        $this->errors = array();
    }
    ...
    public function resize($data) {
        ...
        //$source = ($mime == 'image/png') ? imagecreatefrompng($data['file']['name']) : ($mime == 'image/jpg' || $mime == 'image/jpeg') ? imagecreatefromjpeg(BASEURL . 'resource/users/profile_photo/' . $data['file']['name']) : imagecreatefromgif(BASEURL . 'resource/users/profile_photo/' . $data['file']['name']);
        $source = ($mime == 'image/png') ? imagecreatefrompng($data['file']['name']) : imagecreatefromstring(file_get_contents($data['file']['tmp_name']));
        //$filename = basename($data['file']['name']);

        $filename_array = explode('.', $data['file']['name']);
        $ext = end($filename_array);
        $ext = strtolower($ext);
        $filename = $name = uniqid() . '_' . time() . '.' . $ext;
    ...
    public function crop($data = array()) {
        // Validates params
        if (!isset($data['file']) || !isset($data['w']) || !isset($data['h']) || !isset($data['x']) || !isset($data['y']) || !isset($data['output'])) {
        ...
        // Crop image
        imagecopyresampled($dst_img, $src_img, 0, 0, (int) $data['x'], (int) $data['y'], (int) $data['w'], (int) $data['h'], (int) $data['w'], (int) $data['h']);
